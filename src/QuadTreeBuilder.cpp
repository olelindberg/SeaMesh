...
QuadTreeBuilder::QuadTreeBuilder(int levelmax, double lengthmin)
    : _levelmax(levelmax), _lengthmin(lengthmin) {}

void QuadTreeBuilder::create(std::shared_ptr<QuadTree> &tree) {
  this->create(tree, tree->level, _levelmax);
}

void QuadTreeBuilder::create(std::shared_ptr<QuadTree> &tree, int level,
                             int levelmax) {
  ...
  if (level < levelmax && tree->lengthx() > _lengthmin &&
    ...
    this->create(tree->trees[0], level + 1, levelmax);
    ...
    this->create(tree->trees[1], level + 1, levelmax);
    ...
    this->create(tree->trees[2], level + 1, levelmax);

    // Sub tree 3:
    tree->trees[3] =
        std::make_shared<QuadTree>(center, tree->upper, level + 1, 3);
    tree->trees[3]->set_parent(tree);
    this->create(tree->trees[3], level + 1, levelmax);
  }
}

void QuadTreeBuilder::subdivide(std::shared_ptr<QuadTree> &tree) {
...
void QuadTreeBuilder::balance(std::shared_ptr<QuadTree> &tree) {
