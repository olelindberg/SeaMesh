...
public:
  ...
  static std::shared_ptr<polygon_t>
  variableRadiusCircularPolygon(double x0, double y0, double radius,
                                int numberSegments, bool isInner = true) {
    std::cout << "Creating circular inner boost polygon ...\n";
    auto polygon = std::make_shared<polygon_t>();
    if (isInner)
      polygon->inners().resize(1);
    ...
    for (int i = 0; i < numberSegments + 1; ++i) {
      auto r = radius * (0.75 * (std::cos(2.0 * dtheta * i) + 1.0) + 0.5);
      auto x = x0 + r * std::cos(dtheta * i);
      auto y = y0 + r * std::sin(dtheta * i);
      if (isInner)
        boost::geometry::append(polygon->inners().back(), point_t(x, y));
      else
        boost::geometry::append(polygon->outer(), point_t(x, y));
